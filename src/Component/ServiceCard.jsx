const ServiceCard = ({ service }) => {
 ...
 const {serviceName, description, price, imageURL, serviceArea, userImage, userName} = service;
      ...
      <div className="border-r border-b border-l border-grey-light lg:border-l-0 lg:border-t lg:border-grey-light bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="text-sm">
            {/* Author name */}
            <p className="leading-none text-emerald-500 bg-emerald-100/60 px-3 py-1 rounded-full">{userName}</p>
            {/* Date
            <p className="text-grey-dark">Aug 18</p> */}
          </div>
          </div>
          <div>
              <button className="btn bg-[#3498db] flex-grow text-[#ffffff]">View Details</button>
          </div>
        </div>
      </div>
    </div>
